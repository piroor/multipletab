# 更新履歴
 ...
 - 3.2.3 (2026.7.3)
   * 既定の設定に戻した後に実際の動作が設定通りにならない場合があったのを修正（3.2.2での後退バグ）
 - 3.2.2 (2026.6.30)
   * タブのアイコンが表示されなくなっていたのを修正
   * 自動的に隠れるスクロールバーでタブのクローズボックスがクリック不能にならないように表示位置をずらした
   * 設定の同期およびリセット時にメッセージ処理の無限ループが発生する可能性があったのを修正
 - 3.2.1 (2025.5.24)
   * TST 4.1.7以降でクローズボックスを長押しからのドラッグ操作で選択できなくなっていたのを修正
   * Firefoxのいくつかのバージョンにおいて、タブを長押しからのドラッグ操作で選択できない問題に対処（タブを選択中であることを示すにあたり、擬似要素を動的に追加しないようにした）
 - 3.2.0 (2025.3.13)
 ...
 - 3.1.10 (2021.10.1)
   * `zh_CN`言語リソース更新（by [NightSharp](https://github.com/NightSharp), thanks!）
 - 3.1.9 (2020.9.29)
   * 権限に関する通知のクリックで設定画面に正しく遷移するようにした
